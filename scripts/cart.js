...
(function() {
    'use strict';
    if (window.__CART_SCRIPT_INIT__) return;
     ...
     */
    function addToCart(productId, productName, productPrice, quantity, productOldPrice) {
        quantity = Math.max(1, parseInt(quantity, 10) || 1);
        const cart = getCart();
        const existingItemIndex = cart.findIndex(item => item.id === productId);
        const normalizedOld = parseFloat(productOldPrice);
        const oldPrice = !isNaN(normalizedOld) && normalizedOld > productPrice ? normalizedOld : null;

        if (existingItemIndex !== -1) {
            cart[existingItemIndex].quantity += quantity;
            if (oldPrice !== null) {
                cart[existingItemIndex].old_price = oldPrice;
            }
        } else {
            cart.push({
                id: productId,
    ...
    function init() {
        ...
        document.addEventListener('click', function(e) {
            ...
            if (addToCartBtn) {
                ...
                const productOldPrice = parseFloat(addToCartBtn.getAttribute('data-product-old-price'));
                let quantity = 1;
                ...
                if (productId && productName && !isNaN(productPrice)) {
                    addToCart(productId, productName, productPrice, quantity, productOldPrice);
                }
            }
        });
    }
