...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_feedback'])) {
    ...
    if (empty($subject) || empty($message)) {
        $feedback_error = 'Заполните все поля';
    } else {
        try {
            $stmt = $pdo->prepare("INSERT INTO feedback (name, email, subject, message) VALUES (?, ?, ?, ?)");
            $stmt->execute([$user['name'], $user['email'], $subject, $message]);
            $feedback_success = 'Сообщение отправлено! Мы свяжемся с вами в ближайшее время.';
        } catch (PDOException $e) {
            $feedback_error = 'Ошибка при отправке сообщения';
...
// Получаем избранные товары (из localStorage через JS)
?>
<!DOCTYPE html>
<html lang="ru">
<head>
    ...
    <title>Личный кабинет — PENPOINT</title>
    <style>
        .account-page {
            display: grid;
    ...
    <main class="main">
        <div class="account-page">
            <nav class="account-nav">
                <h2 class="account-nav__title">Настройки профиля</h2>
            ...
            <div class="account-content">
                ...
                <div class="account-section <?php echo $current_tab === 'favorites' ? 'account-section--active' : ''; ?>" id="favorites-section">
                    <h2 class="account-content__title">Избранные товары</h2>
    ...
    <script>
        ...
        function loadFavorites() {
            ...
            if (favorites.length === 0) {
                grid.innerHTML = '<div class="empty-state"><div class="empty-state__icon">❤️</div><p>Список избранного пуст</p></div>';
                return;
            }
            ...
            fetch('/api/products.php?ids=' + favorites.join(','))
                ...
                .then(products => {
                    ...
                    grid.innerHTML = products.map(product => `
                        <article class="product-card">
                            ...
                            <button type="button" class="product-card__add-to-cart" data-product-id="${product.id}" data-product-name="${product.name}" data-product-price="${product.price}">
                ...
                .catch(error => {
                    ...
                    grid.innerHTML = '<div class="empty-state"><p>Ошибка при загрузке избранного</p></div>';
                });
        }
