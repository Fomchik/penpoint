...
$discount_percent = $product ? ($discount_map[(int)$product['id']] ?? 0) : 0;
$price_old = $product ? (float)$product['price'] : 0;
$price_new = $discount_percent ? round($price_old * (1 - $discount_percent / 100), 2) : $price_old;
...
$related_products = $product ? get_related_products($product['id'], 3) : [];
...
<head>
    ...
    <link rel="stylesheet" href="/styles/product-card.css"><!-- карточки в \"С этим покупают\" -->
    <link rel="stylesheet" href="/styles/product-page.css">
    <title><?php echo $product ? htmlspecialchars($product['name']) : 'Товар не найден'; ?> — PENPOINT</title>
</head>
...
<body>
                ...
                <div class="product-page__top">
                    <div class="product-page__gallery">
                        <?php if ($discount_percent): ?>
                            <span class="product-page__badge"><?php echo (int)$discount_percent; ?>%</span>
                        <?php endif; ?>
                        ...
                        <img src="<?php echo htmlspecialchars($image); ?>" alt="<?php echo htmlspecialchars($product['name']); ?>" class="product-page__image">
                    </div>
                    ...
                    <div class="product-page__info">
                        ...
                        <div class="product-page__prices">
                            <?php if ($discount_percent): ?>
                                <span class="product-page__price-old"><?php echo format_price($price_old); ?></span>
                            <?php endif; ?>
                            <span class="product-page__price-new"><?php echo format_price($price_new); ?></span>
                        </div>
                        ...
                        <div class="product-page__stock">
                            ...
                            <span>В наличии: <b><?php echo (int)$product['stock_quantity']; ?> шт</b></span>
                        </div>
                                ...
                                <div class="product-page__tab-content" data-content="specs">
                                    <table style="width: 100%; border-collapse: collapse;">
                                        ...
                                        <tr style="border-bottom: 1px solid #e5e5e5;">
                                            ...
                                            <td style="padding: 12px 0;"><?php echo (int)$product['stock_quantity']; ?> шт.</td>
                                        </tr>
                                        <tr style="border-bottom: 1px solid #e5e5e5;">
                                            <td style="padding: 12px 0; font-weight: 600; color: #666;">Цена</td>
                                            <td style="padding: 12px 0;"><?php echo format_price($price_new); ?></td>
                                        </tr>
                                    </table>
                                </div>
            ...
            <section class="product-page__also-buy">
                ...
                <div class="product-page__also-buy-grid">
                    <?php foreach ($related_products as $p): ?>
                        ...
                        $p_discount = $discount_map[(int)$p['id']] ?? 0;
                        $p_price_old = (float)$p['price'];
                        $p_price_new = $p_discount ? round($p_price_old * (1 - $p_discount / 100), 2) : $p_price_old;
                        ?>
                        <article class="product-card">
                            <?php if ($p_discount): ?>
                                <span class="product-card__badge product-card__badge--discount"><?php echo (int)$p_discount; ?>%</span>
                            <?php endif; ?>
    ...
    <script>
        // Tab switching
        document.querySelectorAll('.product-page__tab').forEach(tab => {
            tab.addEventListener('click', function() {
                const tabName = this.getAttribute('data-tab');
                
                // Update tabs
                document.querySelectorAll('.product-page__tab').forEach(t => t.classList.remove('product-page__tab--active'));
                this.classList.add('product-page__tab--active');
                
                // Update content
                document.querySelectorAll('.product-page__tab-content').forEach(c => {
                    c.classList.remove('product-page__tab-content--active');
                });
        ...
        // Color selection
        document.querySelectorAll('.product-page__color').forEach(colorBtn => {
            colorBtn.addEventListener('click', function() {
                document.querySelectorAll('.product-page__color').forEach(c => c.classList.remove('product-page__color--active'));
